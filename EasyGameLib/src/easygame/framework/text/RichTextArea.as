...
*   构造方法:
	...
	用法示例 usage：
		...
		_richTextArea.configXML = <root>
									...
									<icon iconUrl='img/engine/1.jpg' iconType ="jpg" iconStr=":o"/>
									<icon iconUrl='img/engine/2.gif' iconType ="jpg" iconStr=":)"/>
								</root>;
		_richTextArea.x = 0;
		_richTextArea.y = 0;
...
package easygame.framework.text
{
	...
	{
		//private var _textField:BorderTextField;
...
//		private var _gifPlayer:GifPlayer;
		...
		public function RichTextArea($w:int, $h:int, dtf:TextFormat=null) 
		{
//			_this = this;
			
			_spriteMap = new [];
			_configXML = null; //new XML();
...
//			trace(_cacheTextField.autoSize);
			
			//_textField = new BorderTextField();
			...
			//defaultTextFormat = TextFormatManager.instance.defaultTextFormat;
...
//			_textField.filters = [FilterManager.Instance.black];
			_defaultFormat = dtf;
			if (_defaultFormat) {
				_defaultFormat.size = 12;
				defaultTextFormat = _defaultFormat;
			}
			
			_spriteMask = new Sprite();
			_spriteMask.mouseChildren = false;
			_spriteMask.mouseEnabled = false;
...
//			_this.addEventListener(Event.UNLOAD, unloadHandler);
		...
		private function unloadHandler(evt:Event):void
		{
			...
			this.removeChild(_spriteContainer);
			this.removeChild(_spriteMask);
		}
